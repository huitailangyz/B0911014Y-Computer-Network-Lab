...
void free_tcp_sock(struct tcp_sock *tsk)
{
	...
	tsk->ref_cnt -= 1;
	if (tsk->ref_cnt <= 0) {
		log(FINISH, "free tcp sock: ["IP_FMT":%hu<->"IP_FMT":%hu].", \
				HOST_IP_FMT_STR(tsk->sk_sip), tsk->sk_sport,
...
int tcp_sock_connect(struct tcp_sock *tsk, struct sock_addr *skaddr)
{
	...
	log(DEBUG, "connect tcp sock: ["IP_FMT":%hu<->"IP_FMT":%hu].",
		HOST_IP_FMT_STR(tsk->sk_sip), tsk->sk_sport,
		HOST_IP_FMT_STR(tsk->sk_dip), tsk->sk_dport);
	// [2] hash the tcp sock into bind_table
	if (tcp_bind_hash(tsk) < 0)
		return -1;
...
{
	//fprintf(stdout, "TODO: implement %s.\n", __func__);
	// [1] set backlog (the maximum number of pending connection requst)
	tsk->backlog = backlog;

	// [2] switch the TCP_STATE
	tcp_set_state(tsk, TCP_LISTEN);

	// [3] hash the tcp sock into listen_table
	if (tcp_hash(tsk) < 0)
		return -1;
	...
	switch (tsk->state) {
		...
		case TCP_ESTABLISHED:
			if (!list_empty(&tsk->snd_buffer)){
				tsk->wait_finish = 1;
				log(FINISH, "All data packet have been sent. Wait for close the socket.");
				sleep_on(tsk->wait_send);
			}
			log(FINISH, "Begin for close the socket.");
...
{	
	...
	tsk->rcv_wnd += buf_len;
	log(DEBUG, "The tsk->rcv_wnd increse to %d", tsk->rcv_wnd);
	pthread_mutex_unlock(&tsk->lock);
	if (tsk->rcv_wnd == buf_len) {
		tcp_send_control_packet(tsk, TCP_ACK);
		log(DEBUG, "Tell the peer to send again.");
	}
	//log(DEBUG, "Leave %s", __func__);
	return buf_len;
}
